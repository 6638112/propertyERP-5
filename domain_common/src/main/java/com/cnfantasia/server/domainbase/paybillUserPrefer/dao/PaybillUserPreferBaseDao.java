package com.cnfantasia.server.domainbase.paybillUserPrefer.dao;
 ...
 */
@Repository
public class PaybillUserPreferBaseDao extends AbstractBaseDao implements IPaybillUserPreferBaseDao{
	/**
	 * 根据条件查询(账单优惠用户关系表)信息
	 * @param paramMap
	...
	public int selectPaybillUserPreferCount(Map<String,Object> paramMap,boolean isDim){
		//if(paramMap!=null){paramMap = new HashMap<String, Object>();}
		if(paramMap!=null){paramMap.put(QUERY_TYPE_IF_DIM, isDim);}
		return sqlSession.selectOne("paybillUserPreferBase.select_paybillUserPrefer_count", paramMap);
	}
	/**
	 * 往(账单优惠用户关系表)新增一条记录
	...
	public int insertPaybillUserPrefer(PaybillUserPrefer paybillUserPrefer){
		return sqlSession.insert("paybillUserPreferBase.insert_paybillUserPrefer",paybillUserPrefer);
	}
	/**
	 * 批量新增(账单优惠用户关系表)信息
	...
	public int insertPaybillUserPreferBatch(List<PaybillUserPrefer> paybillUserPreferList) {
		return sqlSession.insert("paybillUserPreferBase.insert_paybillUserPrefer_Batch",paybillUserPreferList);
	}
	...
	public int updatePaybillUserPrefer(PaybillUserPrefer paybillUserPrefer){
		return sqlSession.update("paybillUserPreferBase.update_paybillUserPrefer", paybillUserPrefer);
	}
	/**
	 * 批量更新(账单优惠用户关系表)信息
	 * @param paybillUserPreferList
	...
	public int updatePaybillUserPreferBatch(List<PaybillUserPrefer> paybillUserPreferList) {
		return sqlSession.update("paybillUserPreferBase.update_paybillUserPrefer_Batch", paybillUserPreferList);
	}
