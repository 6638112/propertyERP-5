...
public class OmsUserBaseDao extends AbstractBaseDao implements IOmsUserBaseDao{
	...
	public int insertOmsUserBatch(List<OmsUser> omsUserList) {
		if (omsUserList == null || omsUserList.isEmpty()) {
			return 0;
		}
		int total = 0;
		int size = omsUserList.size();
		int batch = size / batchSize + (size % batchSize == 0 ? 0 : 1);
		for (int i = 0; i < batch; i++) {
			List<OmsUser> batchList = omsUserList.subList(i * batchSize, (i == batch - 1) ? size : i * batchSize + batchSize);
			total += sqlSession.insert("omsUserBase.insert_omsUser_Batch", batchList);
		}
		return total;
	}
