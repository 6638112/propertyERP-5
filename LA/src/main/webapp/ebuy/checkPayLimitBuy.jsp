	...
	<section id="wrapBox" class="sectionBox bggrey pos-relative minheight100">
		<section id="tabBox" class="tabBox pos-relative minheight100 zindex10">
			<section class="sectionBox addressInfo f14">
		        ...
		        <div class="m1510 displaybox">
			        ...
			        <ul class="boxflex01">
				        <!-- 促销商品收货地址 -->
				        <c:if test="${param.jfqsource == 'limitbuy'}">
				            <a class="white" href="../address/qryDeliveryAddressList.do?fromWhere=limitbuy">
				            	<li class="f16">
			    ...
			    <c:if test="${empty addresResponse.dataValue or empty addresResponse.dataValue.userName }">
			        <ul class="p10">
			            <a class="white" href="../address/addAddressInfo.do?fromWhere=limitbuy">
			...
			<c:set var="limitBuyData" value="${jsonResponse.dataValue.limitBuyInfo}" scope="page" />
			<section class="sectionBox payList">
			   <div class="pay-list-info mleft10">
			       <h3 class="item-sort">${limitBuyData.merchantName }</h3>
			       <div class="info ptb10 borderbottomgrey">
			            <div class="tbl">
			                <span class="item-img-small01 inline-table"><img src="${limitBuyData.limitBuyPic}<c:choose><c:when test="${fn:contains(limitBuyData.limitBuyPic, '?')}">&</c:when><c:otherwise>?</c:otherwise></c:choose>x-oss-process=image/resize,m_fill,w_110,h_110,limit_0/format,jpg/interlace,1/quality,q_90" /></span>
			                <span class="item-info tbl-cell">
			                    <div class="f14">${limitBuyData.limitBuyTitle }</div>
			                    <div class="message"></div>
			                </span>
			                <span class="price-num tbl-cell">
			                	<span class="f14">￥<span class="limitBuyPrice">${limitBuyData.limitBuyPrice }</span></span><br/><span class="grey">x<span class="item-qty">${param.buyNum}</span></span>
			                </span>
			                <input type="hidden" name="productIdQty"
			                 data-ptId="${limitBuyData.productId}" data-ptTotal="${limitBuyData.limitBuyPrice * param.buyNum}" />
			            </div>
			        </div>
			        ...
			        <ul class="item-price deliver-box displaybox p210 noborder">
			            <li class="left grey displaybox">
				            <div class="item-check-box item-checked mleft_20 <c:if test="${limitBuyData.signalMer_Deliv_Info == '到店自提' }">item-no-deliver</c:if>" data-merchantid="${limitBuyData.limitBuyId }"></div>
							<div class="boxflex01 deliv-Info">${jsonResponse.dataValue.signalMer_Deliv_Info }
								<c:if test="${limitBuyData.signalMer_Deliv_Info == '到店自提' }">
									(${jsonResponse.dataValue.merchantAddress })
								</c:if>
							</div>
						</li>
						<c:if test="${limitBuyData.signalMer_Deliv_Info != '到店自提' }">
			            	<li class="boxflex01 t-right minwidth90">运费：￥<span class="item-deliver-fee has-deliver-fee">${jsonResponse.dataValue.signalMer_Fee }</span></li>
			            </c:if>
						<c:if test="${limitBuyData.signalMer_Deliv_Info == '到店自提' }">
			            	<li class="boxflex01 t-right minwidth90">免运费<span class="item-deliver-fee has-deliver-fee dsn">0.00</span></li>
			            </c:if>
			        </ul>
			        <c:if test="${jsonResponse.dataValue.merchantType == 2 && limitBuyData.signalMer_Deliv_Info != '到店自提' }">
				        <ul class="item-price deliver-box displaybox p210">
				            <li class="left grey displaybox">
					            <div class="item-check-box item-no-deliver mleft_20" data-merchantid="${limitBuyData.limitBuyId }"></div>
								<div class="boxflex01">到店自提(${jsonResponse.dataValue.merchantAddress })</div>
							</li>
			        ...
			        <ul class="item-price item-sub-total">
			            <li class="left">共  ${param.buyNum } 件商品</li>
			            <li class="right">应付：<span class="red bold f16">￥<span id="merchant-id-${limitBuyData.merchantId }" data-storeid="${limitBuyData.merchantId }" class="merchant-price">${limitBuyData.limitBuyPrice * param.buyNum + jsonResponse.dataValue.signalMer_Fee }</span></span></li>
			        </ul>
			    </div>
			</section> 
			...
			<section class="sectionBox borderbottomgrey">
				<ul class="item-price noborder p10">
			        ...
			        <li id="product-total" class="right">￥${limitBuyData.limitBuyPrice * param.buyNum }</li>
			    </ul>
				<ul class="item-price noborder p10">
			        <li class="left grey">运费合计</li>
			        <li class="right">+￥<span id="transport-total">${jsonResponse.dataValue.signalMer_Fee}</span></li>
			    </ul>
				<ul class="item-price noborder p10">
			    ...
			    <div class="exchang-fixed displaybox">
			        <div class="boxflex01 bggradient p010">
				        ...
				        <span class="red bold f16" id="shouldPay">
							<fmt:formatNumber value="${limitBuyData.limitBuyPrice * param.buyNum + jsonResponse.dataValue.signalMer_Fee - firstSavedAmount}" pattern="0.00" />
						</span>
			        </div>
			        <div class="btn-check-pay bgred">
		        		<a id="btnCheckPay" class="white disblock" href="javascript:void(0)" onclick="limitBuyToPay();">确认付款</a>
			        </div>
			    </div>
		    </section>
		</section>
		...
		<section class="sectionBox wrap-bg pop-box02 dsn">
			<div class="tips-box">
				...
				<ul class="displaybox">
					<li class="boxflex01 ptb5 t-center red pay-check-btn f16 lineheight36"><a class="disblock red" href="../address/addAddressInfo.do?fromWhere=limitbuy">立即填写</a></li>
				</ul>	
			</div>
		</section>
...
<div class="prize-box dsn">
	...
	<section class="sectionBox">
	    <ul class="item-price">
	        ...
	        <li class="right"><span class="red bold f16">￥ <span class="items-total">${limitBuyData.limitBuyPrice * param.buyNum + jsonResponse.dataValue.signalMer_Fee }</span></span></li>
	    </ul>
	    <%--<ul class="item-price">
...
<script src="../js/ebuy.common.js?20171114a"></script>
<script>
	$(function(){
    ...
    function limitBuyToPay(){
	  ...
	  var dataParams = {detailUrl: '/limitBuy/confirmPay.json', isSelfGet: $('[name=isSelfGet]').val()*1, buyNum: $('.item-qty').text()*1, linkPhone: '${addresResponse.dataValue.userPhone }', address: '${addresResponse.dataValue.addressTotal }', limitBuyId: '${limitBuyData.limitBuyId }', linkName: '${addresResponse.dataValue.userName }', couponId: couponId };  
	  ...
	  jQuery.ajax({
            ...
            success:function(data) {
           	   ...
           	   if(shouldPayAmt<0.000001){//100%用券支付成功
           		   ...
           		   window.location.href = "../order/viewOrderDetail.do?orderId="+orderId+"&isNeedSendMsg=1";
           		   return false;
           	   }
