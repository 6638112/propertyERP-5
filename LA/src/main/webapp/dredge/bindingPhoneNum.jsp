...
<title>绑定手机号码</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/dredge/css/recommendaward.css">
</head>
...
<section id="wrapBox" class="sectionBox bggrey pos-relative minheight100">
	<form class="inputform">
			<div class="t-center ptb20 borderbottomgrey bggrey">
				<div class="f18 mtop10 pb10">请输入您的手机号</div>
			</div>
		    <ul class="register-list borderbottomgrey binding-phone-num-box pos-relative">
		        <li class="borderbottomgrey">
		            <div class="displaybox checkPhoneNumBox">
		                ...
		                <div class="boxflex01 f16 grey"><input id="validPhoneNum" class="input-text wp100" type="text" placeholder="输入手机号码" value="" maxlength="11" datatype="m" nullmsg="请输入手机号码" errormsg="请输入正确的手机号码格式" /></div>
	...
	<div class="tips-box tips-done bounceInDown animated1s dsn">
		<img class="rotateZoomIn animated1s delay" src="images/icon-tips-done.png"/><br>绑定成功
	</div>
</section>
	...
	$(".inputform").Validform({
		...
		beforeSubmit:function(){
			...
			jQuery.ajax({//绑定手机，该接口有校验验证码功能
				  url: "../login/bindPhone.json",
				  type: 'post',
				  cache: false,
				  dataType:"json",
				  data:{"detailUrl":"/user/bindPhone.json", "newPhone":$('#validPhoneNum').val(), "newValicode":$('#verifyCode').val()},
				  success:function(data){
					if(data.status =="0000"){
						// 成功 
						//$('.tips-box').show();
						//setTimeout(function(){window.location.href = 'accountSetting.jsp?mobile=' + $('#validPhoneNum').val();},3000);
						window.location.href = '/LA${sessionScope.lastRequestServletPath}';
...
function getValidCode(){
	jQuery.ajax({
		  ...
		  cache: false,
		  dataType:"json",
		  data:{"detailUrl":"/login/getValidateCode.json", "mobile":$('#validPhoneNum').val(), "type":3, "isNeedLogin":"1"},
		  success:function(data){
			  console.log("1111");
			if(data.status =="0000"){
				// 成功
