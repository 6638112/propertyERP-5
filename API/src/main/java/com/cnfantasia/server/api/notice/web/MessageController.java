...
//			messageParamMap.setPicUrl(String.format("/%s%s",fileUploadConfig, fileName));
	...
	public ModelAndView toMessageDetailHtml(HttpServletRequest request) throws Exception {
		...
		ModelAndView mav = new ModelAndView("/notice/noticeModel");
		mav.addObject("title", messageSignalMap.getTitle());
		mav.addObject("time", messageSignalMap.getTime());
		mav.addObject("content", messageSignalMap.getContent());
		...
		//寻求帮助的要查出源数据
		if (messageSignalMap.getSourceId() != null && NoticeDict.Message_Type.NoticeMessage.equals(messageSignalMap.getType())) {
			UserQuestionDetail4Admin userQuestion = userQuestionService.getUserQuestionDetailDetail(messageSignalMap.getSourceId());
			mav.addObject("userQuestionDetail", userQuestion);
			mav.addObject("tagTitle", "管家回复");

			//删除首页寻求帮助消息流
			//未登录避免误删消息流，userId赋值0
			BigInteger tmpUserId = UserContext.getOperIdBigInteger();
			tmpUserId = tmpUserId == null ? BigInteger.ZERO : tmpUserId;
			User user = userBaseService.getUserBySeqId(tmpUserId);
			UserHasHomeMessage hasHomeMessage = new UserHasHomeMessage();
			hasHomeMessage.setUserId(tmpUserId);
			hasHomeMessage.settRoomFId(user == null ? null : user.getDefaultRoomId());
