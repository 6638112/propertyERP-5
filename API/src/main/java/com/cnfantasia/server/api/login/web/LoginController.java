...
//	}
	...
	public JsonResponse getValidateCode(HttpServletRequest request){
		...
		String channelId = HeaderManager.getSubChannelIdLong().toString();
		logger.info("deviceId:" + deviceId + ",channelId:" + channelId);
		...
		} else if (LoginDict.ValiCodeGetType.Property_Card_Charge.compareTo(type) == 0) {
		} else if (LoginDict.ValiCodeGetType.COMMON_PHONE_CHECK.compareTo(type) == 0) {
		} else if (LoginDict.ValiCodeGetType.EBUY_MERCHANT.compareTo(type) == 0) {
		} else {
		...
		String msg = "";
		if(LoginDict.ValiCodeGetType.REGIST.compareTo(type)==0){
			ShortMsgUtil.sendMessage(mobile, "msg.REGIST", validateCode);
		}else if(LoginDict.ValiCodeGetType.FORGET_PASSWORD.compareTo(type)==0){
			ShortMsgUtil.sendMessage(mobile, "msg.FORGET_PASSWORD", validateCode);
		}else if(LoginDict.ValiCodeGetType.LOGIN.compareTo(type)==0){
			ShortMsgUtil.sendMessage(mobile, "msg.LOGIN", validateCode);
//			msg = SmsPropertyUtil.getProperty("msg.LOGIN", validateCode);
		}else if(LoginDict.ValiCodeGetType.UPDATE_PASSWORD.compareTo(type)==0){
			ShortMsgUtil.sendMessage(mobile, "msg.FORGET_PASSWORD", validateCode);
		}else if(LoginDict.ValiCodeGetType.UNBIND_PHONE.compareTo(type)==0){
			ShortMsgUtil.sendMessage(mobile, "msg.UNBIND_PHONE", validateCode);
		}else if(LoginDict.ValiCodeGetType.BIND_CAR.compareTo(type)==0){
			ShortMsgUtil.sendMessage(mobile, "msg.BIND_CAR", validateCode);
        }else if(LoginDict.ValiCodeGetType.Property_Card_Charge.compareTo(type)==0){
        	ShortMsgUtil.sendMessage(mobile, "msg.Property_Card_Ch", validateCode);
        } else if(LoginDict.ValiCodeGetType.COMMON_PHONE_CHECK.compareTo(type)==0){
        	ShortMsgUtil.sendMessage(mobile, "msg.COMMON_PHONE_CHE", validateCode);
		} else if(LoginDict.ValiCodeGetType.EBUY_MERCHANT.compareTo(type)==0){
        	ShortMsgUtil.sendMessage(mobile, "msg.EBUY_MERCHANT", validateCode);
		}
		//3.结果处理
		logger.info("valicode for " + mobile + " is " + validateCode + "。");
...
//		return afterLoginCheck(loginRes);
	 ...
	 * @return
     */
	@RequestMapping(value = "/clearValicode.json")
	@ResponseBody
	public JsonResponse clearValicode(HttpServletRequest request) {
		Integer type = ParamUtils.getInteger(request, "type", null);
		valicodeManager.clearValicode(type);
		return new JsonResponse();
	}

	/**
	 * 注册, 运营后台调用这个接口 
	 * @author wenfq 2016-3-4
	 * @param request
	 * @return
	 */
	@ResponseBody
	@RequestMapping("/regist4ms.json")
	public JsonResponse regist4ms(HttpServletRequest request){
		JsonResponse jsonResponse = new JsonResponse();
		//1.搜集参数
		Integer regType = Integer.parseInt(request.getParameter("regType"));//注册方式
		String account = request.getParameter("mobile");//接收短信验证码的手机号
		...
		//注册
		LoginNo registRes = loginService.regist(account, regType, HeaderConstant.SubChannelId.Jfq_Master_App,version, utmCampaign, utmContent, inviteNo,deviceId,password);
		...
		//3.结果处理
		jsonResponse.setDataValue(registRes);
...
//		}
	 ...
	 * @return
	 */
	@RequestMapping("/resetPwdWithOutValidateCode.json")
	@ResponseBody
	public JsonResponse changePwdWithOutValidateCode(HttpServletRequest request){
		JsonResponse jsonResponse = new JsonResponse();
		//1.搜集参数
		String password = request.getParameter("password");
		BigInteger userId = UserContext.getOperIdMustExistBigInteger();
...
//	}
	...
	public JsonResponse isLogin() {
		JsonResponse jsonResponse = new JsonResponse();
		jsonResponse.putData("isLogin", UserContext.isUserInContext());
		return jsonResponse;
	}
