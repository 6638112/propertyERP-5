...
/**
 * Filename:    WeiXinPayJieFangQuLightAppController.java
 * @version:    1.0.0
 ...
 */
@Controller
@RequestMapping("/weiXinPayJieFangQuLightApp")
public class WeiXinPayJieFangQuLightAppController extends BaseController /*AbstractWeiXinPaymentController*/{
	private Log logger = LogFactory.getLog(getClass());

	private static final String payNotifyUrl = "weiXinPayJieFangQuLightApp/payNotify.json?";
	...
	public JsonResponse prePayRequest(HttpServletRequest request, HttpServletResponse response) throws ParseException, IOException, JDOMException {
		...
		parameters.put("appid", WeiXinPayConstantUtil.getAPP_ID(WeiXinPay_GoalAccType.JieFangQu_LightApp));
		parameters.put("mch_id", WeiXinPayConstantUtil.getPARTNER(WeiXinPay_GoalAccType.JieFangQu_LightApp));
		...
		String encoding = CnfantasiaCommbusiUtil.getSysParaValue("encoding");
		logger.info("ISO8859-1 string: " + new String(reqXml.getBytes(), "ISO8859-1"));
		logger.info("UTF-8 string: " + new String(reqXml.getBytes(), "UTF-8"));
		String packageData = new String(reqXml.getBytes(), StringUtils.isEmpty(encoding) ? "ISO8859-1": encoding);
...
//	}
	...
	private String createMD5Sign(String characterEncoding, SortedMap<Object, Object> parameters) {
		...
		sb.append("key=" + WeiXinPayConstantUtil.getPARTNER_KEY(WeiXinPay_GoalAccType.JieFangQu_LightApp));
