	...
	$(function(){
		...
		if(isAndroid){
			$('#groupBuyBtn').click(function(){
				var groupBuyItemNumSelect = $('.number .itemNum').val(); //拼购数量
				window.groupBuy.groupBuyBtn(fightProductId, ziTiId, groupBuyItemNumSelect);
			});
		}
		//ios
		if(isiOS){
			function groupBuyBtn(param01,param02,param03,param04){
				document.location="jfq://"+param01+":/"+param02+"/"+param03+'/'+param04;
			};
			
			$('#groupBuyBtn').click(function(){
				var groupBuyItemNumSelect = $('.number .itemNum').val(); //拼购数量
				groupBuyBtn('groupBuyBtn', fightProductId, ziTiId, groupBuyItemNumSelect);
			});
		}
