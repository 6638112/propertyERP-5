...
/**
 * @ClassName: HSSFSheeCopy
 * @Date: 2017-02-17 15:20
 * @Auther: yanghua
 * @Description:(sheet复制)
 */
public class HSSFSheeCopy {

    private HSSFSheeCopy() {}
    /**
     * 复制工作表
    ...
    public static void copySheet(HSSFWorkbook workbook, int fromSheetIndex, int toSheetIndex, int startRow, int endRow) {
        HSSFSheet fromSheet = workbook.getSheetAt(fromSheetIndex);
        HSSFSheet toSheet = workbook.getSheetAt(toSheetIndex);
        //合并区域处理
        mergerRegion(fromSheet, toSheet);
        for (int i = startRow; i <= endRow; i++) {
            copyRows(workbook, fromSheetIndex, toSheetIndex, i, i, i);
        }
    }
