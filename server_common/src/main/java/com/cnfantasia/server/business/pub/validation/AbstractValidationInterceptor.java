...
public abstract class AbstractValidationInterceptor extends HandlerInterceptorAdapter {
	...
	// 日志记录器
	private static final Log logger = LogFactory.getLog(AbstractValidationInterceptor.class);
		...
		if (ValidationContext.checkValidate(url)) {
			...
			if (null != regulations && ! regulations.isEmpty()) {
				...
				for (Entry<String, Object> entry : regulations.entrySet()) {
					...
					// 交给执行器进行处理
					value = obtainExpressionExecutor().execute(field, value, expression);
	...
	public abstract ExpressionExecutor obtainExpressionExecutor();
}