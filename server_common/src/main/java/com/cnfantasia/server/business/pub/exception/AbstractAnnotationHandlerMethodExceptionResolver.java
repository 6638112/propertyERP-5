 ...
 * 
 * */
public abstract class AbstractAnnotationHandlerMethodExceptionResolver extends ExceptionHandlerExceptionResolver implements
	...
	public abstract void resolveExceptionPre(HttpServletRequest request, HttpServletResponse response, Object handler,
			Exception exception);
	public abstract void resolveExceptionEnd(HttpServletRequest request, HttpServletResponse response, Object handler,
			Exception exception);
	
	public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object handler,
			Exception exception) {
		ModelAndView returnValue = null;
		resolveExceptionPre(request, response, handler, exception);
		
		HandlerMethod handlerMethod = (HandlerMethod) handler;
		Method method = (handlerMethod == null ? null : handlerMethod.getMethod());
		if (method != null) {
					...
					if (responseStatusAnn != null) {
						...
						if (!StringUtils.hasText(reason)) {
							...
							try {
								...
								logger.debug("AnnotationHandlerMethodExceptionResolver resolveException response.sendError cause exception.", e);
							}
						}
					}
					returnValue = handleResponseBody(returnValue, request, response);
				} catch (Exception e) {
					logger.debug("AnnotationHandlerMethodExceptionResolver  cause exception.", e);
				}
			}
		}
		...
		resolveExceptionEnd(request, response, handlerMethod, exception);
		return returnValue;
	}
