$(function(){
	...
	$(window).scroll(function(){
		...
		var docHeight = $(document).height() - $(window).height()-10; //当前文档高度
		var $cinemaLists = $('.cinemalists:visible');
	...
	$btnSearch.click(function(event){
		console.log($(event.target), $('.ccc').find('.btn-search'));
		//event.target为原生js对象，$('.ccc').find('.btn-search')为jquery数组，需转换为原生对象才能比较	
		if(event.target == $('.ccc').find('.btn-search')[0]){
			console.log(111);	
		}
	...
	$cinemaSearch.on('keypress', function(event){
		currentVal = $.trim($cinemaSearch.val());
		if(event.keyCode == 13){
			if(!currentVal == ''){
				ajaxSearch();
