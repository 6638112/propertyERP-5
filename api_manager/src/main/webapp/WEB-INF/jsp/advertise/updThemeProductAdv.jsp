...
<body>
<div class="info">
	<form class="inputform" action="<%=basePath%>/adv/updThemeProductAdv.html" method="post">
		<h2>编辑推广商品广告</h2>
		<table class="info-list-02" border="0" cellpadding="0" cellspacing="1">
			<tr>
				...
				<td><input type="text" value="${advertise.tittle}" name="ebuyAdvertise.tittle" class="input_text" maxlength="20" datatype="*" placeholder="请填写广告名称" nullmsg="请填写广告名称！"></td>
			</tr>
			<tr>
				<td><div class="list-name"><span class="red">*</span> 广告类型：</div></td>
				<td><input type="radio" name="advType" value="1" data-val="1" datatype="*" nullmsg="请选择广告类型！" <c:if test="${advertise.code == 'EBUY_AD'}">checked</c:if>>超市首页
					<input class="mleft20" type="radio" name="advType" value="3" data-val="3" datatype="*" nullmsg="请选择广告类型！" <c:if test="${advertise.code == 'LA_EBUY'}">checked</c:if>> 轻应用首页
					<input class="mleft20" type="radio" name="advType" value="4" data-val="4" datatype="*" nullmsg="请选择广告类型！" <c:if test="${advertise.code == 'MAIN_BUSINESS_AD'}">checked</c:if>> 首页弹框
					<input class="mleft20" type="radio" name="advType" value="5" data-val="5" datatype="*" nullmsg="请选择广告类型！" <c:if test="${advertise.code == 'EBUY_THEME'}">checked</c:if>> 社区店主题活动
					<input class="mleft20" type="radio" name="advType" value="6" data-val="6" datatype="*" nullmsg="请选择广告类型！" <c:if test="${advertise.code == 'DREDGE_THEME'}">checked</c:if>> 到家主题活动
				</td>
			</tr>
			<tr>
				<td><div class="list-name"><span class="red">*</span> 推广时间：</div></td>
				<td>
					<input type="text" value="${fn:substring(advertise.startTime, 0, 16)}" name="ebuyAdvertise.startTime" class="input_text icon_dt pp" id="date05" placeholder="请选择起始时间" datatype="dateTime" nullmsg="请选择起始时间！"> 至
					<input type="text" value="${fn:substring(advertise.endTime, 0, 16)}" name="ebuyAdvertise.endTime" class="input_text icon_dt pp" id="date06" placeholder="请选择结束时间" datatype="dateTime" nullmsg="请选择结束时间！">
				</td>
			</tr>
			<tr class="swap-con swap-val-5 swap-val-6">
				<td><div class="list-name"><span class="red">*</span> 活动说明：</div></td>
				<td>
					<input type="text" name="ebuyAdvertise.desc" value="${advertise.desc}" class="input_text" maxlength="40" datatype="*" placeholder="请填写活动说明" nullmsg="请填写活动说明！">
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="ebuyAdvertise.frequency" class="select_normal">
						...
						<option value="1" <c:if test="${advertise.frequency == 1}">selected</c:if>>一天一次</option>
			...
			<tr class="city-con01 swap-info swap-val-2">
				...
				<td>
					...
					<input type="hidden" value="${fn:length(products)}" class="select-input01 input_text w80 dsn" datatype="*" nullmsg="请选择商品！" />
				</td>
			</tr>
			<tr>
			...
			<tr class="search-input <c:if test="${areaType == 1}">dsn</c:if>">
				<td></td>
				<td>
					<input type="text" maxlength="8" class="input_text w120" placeholder="请输入关键字" />
			...
			<tr class="city-con <c:if test="${areaType != 2}">dsn</c:if>">
				<td><div class="list-name">搜索结果：</div></td>
				<td>
			...
			<tr class="area-con <c:if test="${areaType != 3}">dsn</c:if>">
				<td><div class="list-name">搜索结果：</div></td>
				<td>
					<ul class="address-list search-box">
					</ul>
...
<script type="text/javascript" src="js/Validform_v5.3.2.js?v20160726"></script>
...
<script type="text/javascript">
	...
	$(function(){
		...
		var selectCountNum01 = ${fn:length(products)};
		...
		$('.addressAreaSelect').change(function(){
			...
			if(curVal == '' || curVal == 1){
				...
				selectCountNum = ${fn:length(areas)};
			}else if(curVal == 2){
				$('.area-con').hide();
