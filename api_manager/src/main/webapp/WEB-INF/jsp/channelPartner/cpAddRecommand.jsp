...
<title>合伙人管理-合伙人资料-新增物业公司推荐</title>
...
<body>
<div class="info">
	<form class="inputform" action="../channelPartner/saveNewChannelPartnerRecommend.html">
        ...
        <table class="info-list-02" border="0" cellspacing="1" cellpadding="0">
                <tr>
                        ...
                        <td>
                        <input type="text" class="input_text pp" name="pcName" value="" datatype="*" nullmsg="请输入物业公司名称！" onchange="validatePCName(this);">
                        </td>
                      </tr>
                      <tr>
                        <td align="right"><Span class="red">*</Span> 物业资质：</td>
                        <td>
                        	<select id="aptitude" class="select_normal f14" name="pcQualifications" datatype="*" nullmsg="请选择物业资质！">
	                            <option value="0">请选择</option>
	                            <option value="1" data-num="12000">一级资质</option>
	                            <option value="2" data-num="8000">二级资质</option>
	                            <option value="3" data-num="5000">三级资质</option>
                        	</select>
                        </td>
                      </tr>
                      <tr>
                        <td align="right"><Span class="red">*</Span> 管理小区住户数：</td>
                        <td><input type="text" id="plotNum" name="residentCount" class="input_text pp" maxlength="6" value="" datatype="n" nullmsg="请输入管理小区住户数！" errormsg="管理小区住户数由数字组成！"> 户</td>
                      </tr>
                      <tr>
                        <td align="right">上传物业公司的营业执照：</td>
                        <td><input name="iconBusinessLicense" type="file" class="w150" onchange="checkImgType(this);"></td>
                      </tr>
                      <tr>
                        <td align="right"><Span class="red">*</Span> 公司地址：</td>
                        <td><input type="text" maxlength="100" name="pcAddress" class="input_text pp" value="" datatype="*" nullmsg="请输入公司地址！"></td>
                      </tr>
                      <tr>
                        <td align="right"><Span class="red">*</Span> 公司联系人：</td>
                        <td><input type="text" maxlength="10" name="pcLinkMan" class="input_text pp" value="" datatype="*" nullmsg="请输入公司联系人！"></td>
                      </tr>
                      <tr>
...
(function($){
	...
	//数字滚动效果
	function iNum( Rate, opts){
		var iNum_end = Rate;  //数字
		var iNum_start = 0;
		var oNum = $(opts);
		var iCome = parseFloat(iNum_start);
		
		setInterval(function(){
			if( iNum_start < iNum_end ){
				iNum_start = iNum_start+=iNum_end/160;
				oNum.html(iNum_start.toFixed(2));
				if(iNum_start >= iNum_end){
					iNum_start = iNum_end;
					oNum.html(iNum_start.toFixed(2));
				}
	...
	//计算收益
	$('#incomeCount').click(function(){
		var $aptitude = $('#aptitude');
		var $plotNum = $('#plotNum');
		var aptitudeVal = $aptitude.find('option:selected').attr('data-num');
		var plotNum = $plotNum.val()*20;
		var totalNum = aptitudeVal*1 + plotNum*1;
		$('#aptitudeNum').text(aptitudeVal);
		$('#downLoadNum').text(plotNum);
		$('#totalNum').text(totalNum);
		iNum( aptitudeVal, '.num01');
		iNum( plotNum, '.num02');
		iNum( totalNum, '.num03');
	});
	
	//下一步 不可跳过校验
	$('.info-btn, .skipBtn').click(function(){
