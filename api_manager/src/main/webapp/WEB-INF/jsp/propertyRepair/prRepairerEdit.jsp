	...
	<form class="inputform" method="post" action="../propertyRepair/saveRepairer.html" enctype="multipart/form-data">
        ...
        <table class="info-list-02" border="0" cellpadding="0" cellspacing="1">
          <tr>
            ...
            <td><input type="text" name="repairerName" class="input_text pp" value="${propertyRepairer.name}" datatype="*" nullmsg="请填写员工姓名！" maxlength="6"></td>
          </tr>
          <tr>
            <td align="right"><Span class="red">*</Span> 联系手机：</td>
            <td><input type="text" name="repairerTel" class="input_text pp" value="${propertyRepairer.tel}" datatype="m" nullmsg="请填写联系手机！" maxlength="11"></td>
          </tr>
          <tr>
            <td align="right">大头照：</td>
            <td><div class="uploadPreview" style="background:url(<%=OmsSysParamManager.getImageServerUrl(PathConstants.RepairPicBasePath) + PathConstants.RepairPicBasePath %>${propertyRepairer.headimgurl}); background-size:168px"></div><input type="file" name="headimgurl" class="uploadImage fimg1 w150" /><span class="f12 mar-left15">注：宽度须大于200像素</span></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td><div class="uploadPreview" style="background:url(<%=OmsSysParamManager.getImageServerUrl(PathConstants.RepairPicBasePath) + PathConstants.RepairPicBasePath %>${propertyRepairer.idNumberPic1}); background-size:168px"></div><input type="file" name="idimgurl1" class="uploadImage fimg1 w150"  /><span class="f12 mar-left15">注：宽度须大于200像素</span></td>
          </tr>
          <tr>
            <td align="right">身份证反面：</td>
            <td><div class="uploadPreview" style="background:url(<%=OmsSysParamManager.getImageServerUrl(PathConstants.RepairPicBasePath) + PathConstants.RepairPicBasePath %>${propertyRepairer.idNumberPic2}); background-size:168px"></div><input type="file" name="idimgurl2" class="uploadImage fimg1 w150" /><span class="f12 mar-left15">注：宽度须大于200像素</span></td>
          </tr>
          <tr>
		...
		$(".inputform").Validform({
			...
			beforeSubmit:function(){
				...
				$.ajax({
					...
					data:{mobile: $("input[name=repairerTel]").val(), dwId: $("input[name=propertyRepairerId]").val(),},
					dataType:"json",
					async:false,
