...
<body>
<div class="info">
    <form class="inputform" action="<%=basePath%>/prizeActivityJson/doActivityAdd.json" enctype="multipart/form-data" method="post">
        <h2>新增活动</h2>
        <table class="info-list-02" border="0" cellpadding="0" cellspacing="1">
          ...
          <tr>
            ...
            <td><input type="text" name="name" value="" class="input_text w300" datatype="*" nullmsg="请填写活动名称！" placeholder="请填写活动名称" /></td>
          </tr>
          <tr>
            <td><div class="list-name"><span class="red">*</span> 活动时间：</div></td>
            <td><input type="text" name="startTime" value="" class="input_text icon_dt" id="date01" placeholder="请选择起始时间" datatype="*" nullmsg="请选择起始时间！" /> 至 <input type="text" name="endTime" value="" class="input_text icon_dt" id="date02" placeholder="请选择结束时间" datatype="*" nullmsg="请选择结束时间！" /></td>
          </tr>
          <tr class="list-title">
          ...
          <tr>
            ...
            <td><input type="text" name="shareText" value="" class="input_text pp w500" placeholder="分享链接文案" maxlength="20" datatype="*" nullmsg="请完善商品信息！" /><span class="f12 mar-left15">注：20字以内</span></td>
          </tr>
          <tr>
            <td><div class="list-name"><span class="red">*</span> 分享链接图标：</div></td>
            <td>                
                <input type="hidden" class="input_text w300 img-upload-status" hidden="true" datatype="*" nullmsg="请上传分享链接图标！" />
                <div class="uploadPreview01"><input type="file" name="shareIcon" class="uploadImage02" /><img class="imgPreview" width="82" height="82" src="images/addimg01.jpg" /></div>
                <span class="f12 mar-left15">注：仅限一张图片，最佳尺寸114px*114px</span>
                <div id="picDiv" style="width: 1px; height: 1px;"></div>
            </td>
          </tr>
        </table>
        <div class="mtop40 f14"><span class="red">*</span> <span class="bold">是否开启活动：</span> <input class="mtop3" name="activityStatus" value="2" type="radio"/> 是 <input class="mtop3 mleft20" name="activityStatus" value="1" checked="checked" type="radio" datatype="*" nullmsg="请选择是否开启活动！"/> 否</div>
		  ...
		  <td colspan="2" bgcolor="#f5f5f5">
		      ...
		      <table class="ranking-info" border="0" cellpadding="0" cellspacing="1">
		        <tr class="nobg">
		          ...
		          <td><input type="text" name="${it.id}_total" onclick="qryLeftPriOptCount(${it.id},'potId_${it.id}','<%=basePath%>');" class="input_text w300 optionCountNum" ignore="ignore" datatype="n" nullmsg="请填写派奖额度！" placeholder="该次活动需要使用的数量" /> 
		          	库存<span id="potId_${it.id}" class="optionLeftNum">-</span>份</td>
		        </tr>
		        <tr class="nobg">
		          <td><span class="red">*</span> 日均派发数：</td>
		          <td><input type="text" name="${it.id}_day" class="input_text w300 optionDateNum" ignore="ignore" datatype="n" nullmsg="请填写日均派发数！" placeholder="每天最大派奖份数" /></td>
		        </tr>
		      </table>
		  </td>
		</tr>
...
<script type="text/javascript" src="busiJS/prizeActivity.js?v20151013"></script>
...
(function($){
	...
	$(".inputform").Validform({
		...
		callback:function(data){
			$(".inputform").ajaxSubmit(function(data) {
				...
				if (data.status == '0000') {
					alert("活动添加成功！");
					window.location.href="<%=basePath%>/prizeActivity/activityList.html";
				} else {
					alert(data.message);
    ...
    $('.optionCheckBox').click(function(){
    	if($(this).is(":checked")){    		
    		...
    		var optionDate01 = $(this).parent('td').siblings('td').find('.optionDate01').text().substring(0,16);
    		var optionDate02 = $(this).parent('td').siblings('td').find('.optionDate02').text().substring(0,16);
    		var now = new Date();
    		if(optionDate01 > date01 || optionDate02 < date02){
    			alert('该奖项时间与活动时间不符，请重新选择奖项！');
    ...
    $('.optionCountNum').keyup(function(){
    	...
    	if(optionCountNum < optionDateNum){
    		alert('须大于或等于派奖数量！');
    		$(this).val('');
    	}
    });
