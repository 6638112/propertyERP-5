...
<html>
	...
	<body style="display:none;">
		<div class="info">
		    <h2>新增运营通知</h2> 
		    <form class="inputform" enctype="multipart/form-data" method="post" action="update.html" >
			    ...
			    <table class="info-list-02" style="min-width: 440px;margin-top: 20px;">
					<tr>
						...
						<td class="dpTd2">
				        	...
				        	<label style="margin-right: 2em;" onclick="setSendRange('4')"><input type="radio" name="sendRange" value="4" <c:if test="${m.sendRange==4 }"> checked</c:if> />城市/区县</label>
							<label onclick="setSendRange('3')"><input type="radio" name="sendRange" value="3" <c:if test="${m.sendRange==3 }"> checked</c:if>/>小区</label>
						</td>
					</tr>
					<tr>
						<td class="dpTd1 title">信息类型：</td>
						<td class="dpTd2">
							<label style="margin-right: 2em;"> <input name="msgType" type="radio" value="1" <c:if test="${m.msgType==1 }"> checked</c:if>>短信 </label>
							<input name="msgType" type="radio" value="2" <c:if test="${m.msgType==2}"> checked</c:if> >push
						</td>
					</tr>
					<tr id="jumpTargetTr">
						...
						<td class="dpTd2">
							<select name="jumpTarget">
								<c:forEach items="${oprtConfigList }" var="item">
								  <option value="${item.number }" <c:if test="${m.jumpTarget==item.number }"> selected</c:if> >${item.jumpDescription }</option>
								</c:forEach>
							</select>
						</td>
					</tr>
					<tr id="prdtIdDetailTr">
						...
						<td class="dpTd2">
							<input name="prdtParam" type="text" value="${m.ebuyProductDetail }">
						</td>
					</tr>
					<tr id="blackTr">
						<td class="dpTd1 title">短信黑名单：</td>
						<td class="dpTd2">
							<textarea name="blackList" rows="8" cols="150">${m.blackList }</textarea>
							<br>注意：每个手机号用;分隔
						</td>
					</tr>
					<tr id="whiteTr">
						...
						<td class="dpTd2">
							<textarea name="whiteList" rows="8" cols="150">${m.whiteList }</textarea>
							<br>注意：每个手机号用;分隔
						</td>
					</tr>
					...
					<tr>
						...
						<td class="dpTd2">
							<p id="sendTimeP">时间  <input name="sendTime" type="text" value="${m.sendTime }" class="input_text pp icon_dt" id="date01" title="请选择发送时间""></p>
						</td>
					</tr>
					...
					<tr>
						...
						<td class="dpTd2">
							<input type="text" name="title" datatype="*2-45" nullmsg="请填写信息！" value="${m.title }" errormsg="长度2到45个字符！" class="input_text w240 pp" maxlength="45">
						</td>
					</tr>
					...
					<tr>
						...
						<td class="dpTd2">
							<textarea class="content" rows="10" cols="100" name="content" datatype="*8-500" nullmsg="请填写信息！" errormsg="长度8到800个字符！" class="input_text w240 pp" maxlength="800">
								${m.content }
							</textarea>
						</td>
					</tr>
		        ...
		        <div id="cityRange">
					...
					<display:table name="blockList" id="cityTable" class="info-list-02" requestURI="" >
						<display:column title="<label><a href='javascript:void(0);'><input type='checkbox' onclick='selectAllCityBox(this)'/>全选</a></label>"  sortable="true">
							<input name="blockId" type="checkbox" value="${cityTable.blockId }" 
								<c:if test="${m.sendRange == 4 }">
									<c:forEach items="${sendRangeList }" var="item">
										<c:if test="${cityTable.blockId == item.abId }"> checked</c:if>
									</c:forEach>
								</c:if>
		        ...
		        <div id="gbRange">
					...
					<display:table name="gbList" id="gbTable" class="info-list-02" requestURI="" >
						<display:column title="<label><a href='javascript:void(0);'><input type='checkbox' onclick='selectAllGbBox(this)'/>全选</a></label>"  sortable="true">
							<input name="gbId" type="checkbox" value="${gbTable.id }" 
								<c:if test="${m.sendRange == 3 }">
									<c:forEach items="${sendRangeList }" var="item">
										<c:if test="${gbTable.id == item.gbId }"> checked</c:if>
									</c:forEach>
								</c:if>
							onclick="selectGb(this)"/>
						</display:column>
	...
	$(function(){
			...
			var msgType = $("input[type='radio'][name='msgType']:checked").val();
			msgTypeSwap(msgType);
			...
			// 选中城市行政区的处理			
			$("#cityTable input[name=blockId]:checked").each(function(){
				selectCity(this);
			});
			// 选中小区的处理			
			$("#gbTable input[name=gbId]:checked").each(function(){
				selectGb(this);
			});
		...
		$(document).ready(function () {
	        xheditor = $("textarea.content").xheditor({
	            tools:'',
