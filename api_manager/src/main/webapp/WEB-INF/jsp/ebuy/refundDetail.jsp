...
<div class="info">
          ...
          <tr>
            <td style="padding-left:50px;" width="50%">
            	<table border="0" cellpadding="0" cellspacing="0">
		  		...
		  		<tr><td clase="red">退款凭证：<c:if test="${picList == null}">无凭证图片信息</c:if></td></tr>
		        </table>
            </td>
            <td style="padding-left:50px;" width="50%" valign="top">
		        <div class="bs-example">
		            <table class="info-list" border="0">
		             <c:if test="${refundOrder.refundStatus ==2}"><tr class="red"><td>退款时间：${refundOrder.sys0UpdTime}</td></tr></c:if>
		              <tr class="red"><td>退款粮票：${refundOrder.refundRedEnvelope}</td></tr>
		              <tr class="red"><td>退款现金：${refundOrder.refundMoney}</td></tr>
					  <tr class="red">
						  <td>退款审核：
						  	<c:choose>
								<c:when test="${refundOrder.refundStatus ==2}">退款成功</c:when>
					  ...
					  <c:if test="${refundOrder.refundStatus ==3}">
					  <tr class="red"><td>审核说明：${refundOrder.refundReason}</td></tr>
					  </c:if>
		            </table>
		        </div>
            </td>
        ...
        <display:table class="info-list-02 mtop20" name="refundproductList" id="refundpro" >
        	...
        	<display:column title="商品名称">${refundpro.refundproductName}</display:column>
            ...
            <td style="padding-left:50px;" width="50%">
                <table border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr><td>运单状态：</td>
                        <td>
                            <c:choose>
                                <c:when test="${orderDetail.deliveryStatus == 0}">
                                    <div class="red">未启动</div>
                                </c:when>
                                <c:when test="${orderDetail.deliveryStatus == 1}">
                                    <div class="red">待发货</div>
                                </c:when>
                                <c:when test="${orderDetail.deliveryStatus == 2}">
                                    <div class="red">待收货</div>
                                </c:when>
                                <c:when test="${orderDetail.deliveryStatus == 3}">
                                    <div class="red">确认收货</div>
                                </c:when>
                            </c:choose>
                        </td>
                    </tr>
