...
<div class="info">
    ...
    <div class="bs-example bgebeb">
    	<form action="${pageContext.request.contextPath}/communitySupply/searchTmp.html">
	        <table class="info-list citySelect" border="0">
	          <tr>
	            <td><div class="list-name">店铺名称：</div></td>
	            <td><input name="cstName" type="text" class="input_text w120 pp" value="${param.cstName}"></td>
    ...
    <display:table name="resList" id="row" class="info-list-02 mtop20" cellpadding="0" cellspacing="1" pagesize="10" requestURI="" partialList="true" size="resultSize">
		<display:column title="店铺名称" property="supplyName" />
		<display:column title="店铺地址" property="addressDetail"/>
		<display:column title="类别" property="supplyType" />
		...
		<display:column title="操作" >
			<a class="blue editShopInfo" href="${pageContext.request.contextPath}/communitySupply/viewTmpDetail.html?id=${row.id }">查看</a>
			<c:if test="${row.auditStatus==1 }">
		        <a class="blue checkShopInfo auditHref" href="${pageContext.request.contextPath}/communitySupply/auditCSTmpView.html?id=${row.id }">审核</a>
			</c:if>
		</display:column>
	</display:table>
</div>
