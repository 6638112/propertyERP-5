<%@page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" errorPage="${pageContext.request.contextPath}/error/404.jsp"%>
...
<div class="info">
    ...
    <div class="bs-example bgebeb">
    	<form action="${pageContext.request.contextPath}/communitySupply/searchPSC.html">
  		...
  		<div class="mtop10 t_center btn-search"><input class="input-btn w200" type="submit" value="搜索"></div>
    	</form>
    </div>
    ...
    <display:table name="resList" id="row" class="info-list-02 mtop20" cellpadding="0" cellspacing="1" pagesize="10" requestURI="" partialList="true" size="resultSize">
		...
		<display:column title="推荐时间" property="sys0AddTime" />
		<display:column title="物业公司">${row.propertyCompany.name}</display:column>
		...
		<display:column title="操作" >
			<a class="blue editShopInfo" href="${pageContext.request.contextPath}/communitySupply/viewDetail.html?id=${row.communitySupply.id }">查看</a>
			<c:if test="${row.auditStatus==1 }">
		        <a class="blue checkShopInfo auditHref" href="${pageContext.request.contextPath}/communitySupply/auditPSCView.html?id=${row.id }">审核</a>
			</c:if>
		</display:column>
	</display:table> 
</div>
