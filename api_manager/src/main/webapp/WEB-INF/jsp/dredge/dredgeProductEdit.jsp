...
<html>
	<head>
		...
		<style>
			...
			.red{color:red;}
			.visable{background-color: #E0DDDD;}
	...
	<body>
		<div class="info">
		    <h2>编辑服务商品管理</h2> 
		    <form class="inputform" enctype="multipart/form-data" method="post" action="productUpdate.html">
				...
				<table class="info-list-02" style="width: 440px;min-width: 440px;">
					<tr>
						...
						<td class="dpTd2">
							<select class="select_normal grey" name="cstId" datatype="*" onchange="cstTypeChange()" disabled="disabled">
					...
					<tr>
						...
						<td class="dpTd2">
							<select class="select_normal grey" name="dtId" datatype="*" onchange="dtChange()" disabled>
					...
					<tr>
						...
						<td class="dpTd2">
							<select class="select_normal grey" datatype="*" name="dt2Id" disabled>
					            <option value="">请选择</option>
		        ...
		        <table class="info-list-02">
					<tr>
						...
						<td class="dpTd3">
							<select class="select_normal grey" datatype="*" name="ssId" disabled>
					            <option value="">请选择</option>
					...
					<tr>
						...
						<td class="dpTd3">
							<label style="margin-right: 3em;"><input class="grey" disabled="disabled" readonly="readonly" type="radio" name="payMethod" value="1" <c:if test="${dp.payType==1 }">checked</c:if> />服务前付款</label>
							<label><input type="radio" class="grey" disabled="disabled" readonly="readonly" name="payMethod" value="2" <c:if test="${dp.payType==2 }">checked</c:if> />服务后付款</label>
						</td>
					</tr>
					<tr>
							...
							<table class="info-list-02" id="specTable">
								...
								<tbody id="noBody">
									<c:forEach items="${dp.prdtSpecList }" var="item"> 
										<tr>
											<td><input type="text" class="input_text w120 pp <c:if test="${item.isVisible == 0}">visable</c:if>" maxlength="40" datatype="*2-30" nullmsg="请填写信息！" value="${item.specification }"/>
												<span class="Validform_checktip"></span>
											</td>
											<td><input  type="text" datatype="numberFixTwo" nullmsg="请填写信息！" class="input_text w120 pp <c:if test="${item.isVisible == 0}">visable</c:if>" maxlength="40" value="${item.sellPrice/100 }"/>
											<span class="Validform_checktip"></span></td>
											<td><input type="text" datatype="numberFixTwo" nullmsg="请填写信息！" class="input_text w120 pp <c:if test="${item.isVisible == 0}">visable</c:if>" maxlength="40" value="${item.marketPrice/100 }"/>
											<span class="Validform_checktip"></span></td>
											<td><input type="text"  datatype="*1-8" nullmsg="请填写信息！" errormsg="长度1到8个字符！" class="input_text w120 pp <c:if test="${item.isVisible == 0}">visable</c:if>" maxlength="40" value="${item.priceUnit }"/>
											<span class="Validform_checktip"></span></td>
											<td class="op">
												<input type="hidden" name="dspId" class="input_text w120 pp" maxlength="40" value="${item.id }"/>
												<input type="hidden" name="isVisible" class="input_text w120 pp" maxlength="40" value="${item.isVisible }"/>
												<input type="button" class="input-btn" value="删除" onclick="$(this).parent().parent().remove();"/>
												<input type="button" class="input-btn" value="<c:if test="${item.isVisible == 1}">隐藏</c:if><c:if test="${item.isVisible == 0}">显示</c:if>" onclick="visibleChange(this)"/>
											</td>
										</tr>
									</c:forEach>
								</tbody>
						...
						<td class="pcTd2" id="dragScanItems">
							<c:forEach items="${dp.prdtPics }" var="pic" varStatus="status">
								<div class="uploadPreview01 drag-item marb15">
				                    ...
				                    <input type="file" accept="image/gif,image/jpeg,image/jpg,image/png" name="prdtPic" class="uploadImage02"  onchange="$(this).siblings('input[name=prdtPicHistory]').remove();"/>
				                    <img class="imgPreview" width="82" height="82" src="${pic }" />
				                </div>
			                </c:forEach>
			                <div class="uploadPreview01 scanPic-new-add img-hide marb15 mright6 drag-item">
			                	<div class="img-delete-btn">删除</div>
						...
						<td id="dragIntroduceItems">
							<c:forEach items="${dp.introducePics }" var="pic" varStatus="status">
								<div class="uploadPreview01  drag-item marb15">
				                    ...
				                    <input type="file" accept="image/gif,image/jpeg,image/jpg,image/png" name="introducePic" class="uploadImage02" onchange="$(this).siblings('input[name=introducePicHistory]').remove();" />
				                    <img class="imgPreview" width="82" height="82" src="${pic }" />
				                </div>
				             </c:forEach>
			                <div class="uploadPreview01 introducePic-new-add img-hide marb15 mright6 drag-item">
			                	<div class="img-delete-btn">删除</div>
	...
	<script type="text/javascript">
			...
			$(".inputform").Validform({
				...
				beforeSubmit:function(curform){
					...
					$("#specTable tbody tr").each(function(key,value){
						...
						var isVisible = $(this).find("input[name='isVisible']").val();
						obj.isVisible =  (isVisible == null || isVisible == '' || isVisible == undefined) ? 1 : isVisible;
						specList.push(obj);
					});
		...
		function visibleChange(obj) {
			var status = $(obj).siblings("input[name='isVisible']").val();
			if(status == 1) {//隐藏
				$(obj).parent("td").parent("tr").find("input[type='text']").addClass("visable");
				$(obj).val("显示");
				$(obj).siblings("input[name='isVisible']").val(0)
			} else {//显示
				$(obj).parent("td").parent("tr").find("input[type='text']").removeClass("visable");
				$(obj).val("隐藏");
				$(obj).siblings("input[name='isVisible']").val(1)
			}
		}
