...
<header class="sectionBox fantasia-header order-top-bg">
    ...
    <div class="header-title">商品详情-审核不通过</div>
</header>
<section class="sectionBox item-status-desc bgred item-details-info">${prodDetail.reason}</section>

<form class="inputform" id="inputform" action="saveProduct.html" enctype="multipart/form-data" method="post" onsubmit="return false;">
...
<section class="sectionBox padding-menu pos-relative item-details-info">
        	...
        	<div id="dragItems">
        		<c:forEach var="introducePic" items="${introducePicList}" varStatus="status">
        	     <div class="uploadPreview01 mright6 mtop30 marb20 drag-item" style="background-image: url(<%=OmsSysParamManager.getImageServerUrl("/productPic/") %>/productPic/${introducePic.urlBig}<c:choose><c:when test="${fn:contains(introducePic.urlBig, '?')}">&</c:when><c:otherwise>?</c:otherwise></c:choose>x-oss-process=image/resize,m_fill,w_140,h_140/format,jpg/interlace,1/quality,q_80)">
                    ...
                    <input type="file" accept="image/gif,image/jpeg,image/jpg,image/png" readonly="readonly" name="photoimage" class="uploadImage01" value="${status.index}" indexNum="${status.index}"/>
                </div>
                </c:forEach>
            </div>
    ...
    <ul class="bottom-menu-box displaybox t-center">
    	<li class="p00"><input class="btn-submit btn-next noradius bordertbgrey btnSubmit red" data-param="1" type="button" value="放入仓库" /></li>
    	<li class="p00"><input class="btn-submit bgred noradius btnSubmit" data-param="2" type="button" value="提交出售" /></li>
    </ul>
...
<section id="itemParamInfo" class="sectionBox padding-menu pos-relative hide">
    ...
    <ul class="register-list">
        <li class="overvisible">
        	<div id="dragItems">
        		<c:forEach var="introducePic" items="${introducePicList}" varStatus="status">
        	     <div class="uploadPreview01 mright6 mtop30 marb20 drag-item" style="background-image: url(<%=OmsSysParamManager.getImageServerUrl("/productPic/") %>/productPic/${introducePic.urlBig})">
        	     	<div class="img-delete-btn">删除</div>
                    <div class="img-desc-text">添加描述</div>
                    <div class="img-desc-box dsn"><span class="desc-text-info">文字描述</span><span class="icon-edit"><img width="14" height="14" src="../images/icon-edit.png" /></span></div>
                    <input class="desc-text-input dsn" type="text" name="descText" value="${introducePic.textDesc}" />
...
<script src="../js/jquery.form.js?v08272301"></script>
<script>
	$(function(){
...
<script src="../js/merchant/Validform_v5.3.2.js?0827"></script>
