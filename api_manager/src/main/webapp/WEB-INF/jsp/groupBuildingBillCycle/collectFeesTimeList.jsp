...
<div class="info">
    <h2>收费时间设置</h2>
    <form action="<%=basePath%>/groupBuildingBillCycle/getCollectFeesTimeList.html" method="post">
    ...
    <display:table name="resList" id="row" class="mars info-list-02 mtop20" pagesize="<%=OmsSysParamManager.getOmsSysParaValue(OmsSysParamKey.PageNum) %>" requestURI="" partialList="true" size="resultSize" >
        ...
        <display:column title="操作">
            <a class="blue viewTicket" href="<%=basePath%>/groupBuildingBillCycle/goToCollectFeesCfg.html?gbId=${row.id}&gbName=${row.name}">收费时间管理</a>
        </display:column>
    </display:table>
</div>
</body>
