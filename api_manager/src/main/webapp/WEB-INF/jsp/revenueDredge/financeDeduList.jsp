...
function exp(){
	location.href = "<%=basePath%>/revenueFinanceDedu/exportExcel.html";
}

function onSubmit() {
	$("#searchForm").attr("action", "<%=basePath%>/revenueFinanceDedu/financeDeduList.html");
	$("#searchForm").submit();
}
</script>
</head>
...
<div class="info">
    ...
    <form id="searchForm" name="form_main" action="<%=basePath%>/revenueFinanceDedu/financeDeduList.html" method="post">
	    ...
	    <display:table name="financeDeduRevenueSignalAmountList" id="row" class="info-list-02 mtop20" cellpadding="0" cellspacing="1" pagesize="<%=OmsSysParamManager.getOmsSysParaValue(OmsSysParamKey.PageNum) %>" requestURI="" partialList="true" size="resultSize">
			...
			<display:column title="抵扣月份" property="month" sortable="true" format="{0, date, yyyy-MM}" ></display:column>
