	...
	**/
	...
	PickerHandler.prototype._relocate = function(){
		...
		if ($input != null && $picker.data('isInline') === false) { // Float mode
			...
			var picker_outer_height = 232; //$picker.outerHeight({'margin': true});
			if (!isObj('Number', picker_outer_height)) {
				picker_outer_height = 232; //$picker.outerHeight();
			}
			...
			if(parseInt($(window.parent.document).height()) <=  ($input.offset().top - $(window.parent.document).find(".main-right").scrollTop() + input_outer_height + picker_outer_height +70) ){	
				//iframe内的高度，须通过$(window.parent.document)来获取，.height()不准确，.scrollTop()始终为0。70为顶部黑色块高度
				// Display to top of an input-field
				//$picker.parent().css('top', ($input.offset().top - (input_outer_height / 2) - 232) + 'px');
				$picker.parent().css('top', ($input.offset().top - 2 - picker_outer_height + $(window.parent.document).find(".main-right").scrollTop()) + 'px');
			} else {
			...
			if(($picker.parent().width() + $input.offset().left > $(window).width() && $input.offset().left + $input.outerWidth() > $(window).width()) || $input.offset().left < 0 || ($picker.parent().width() + $input.offset().left > $(window).width() && $input.offset().left + $input.outerWidth() < $(window).width() && $input.offset().left + $input.outerWidth() < $picker.parent().width())) {
				// Display left side stick to window
				$picker.parent().css('left', (($(window).width() - $picker.parent().width()) / 2) + 'px');
			}else if($picker.parent().width() + $input.offset().left > $(window).width() && $input.offset().left + $input.outerWidth() < $(window).width() && $input.offset().left + $input.outerWidth() > $picker.parent().width()) {
				// Display left side stick to window
				$picker.parent().css('left', ($input.offset().left + $input.outerWidth() - $picker.parent().width() + 11) + 'px');
			} else {
	...
	**/
	...
	var draw = function($picker, option, year, month, day, hour, min) {
		...
		if (isTodayButton) {
			...
			//增加关闭按钮
			var $close_btn = $('<a class="icon-close" style="float:right;"><img src="../images/icon-close.png"></a>');
			/*
				This icon resource from a part of "FontAwesome" by Dave Gandy - http://fontawesome.io".
				http://fortawesome.github.io/Font-Awesome/license/
				Thankyou.
			*/
			$link_today.html( '<img src="../images/icon-home.png" />' );
...
//选择日期  格式：年月日 时分，只能选择晚于当前时间
$(function(){
	$('input[id=date05]').appendDtpicker({ 
		"closeOnSelected":true,
		"locale":"cn",
		"autodateOnStart":false,
		"futureOnly": true
...
$(function(){
	$('input[id=date06]').appendDtpicker({ 
		"closeOnSelected":true,
		"locale":"cn",
		"autodateOnStart":false,
		"futureOnly": true
...
//选择日期  格式：年月日，只能选择晚于当前时间
$(function(){
	$('input[id=date07]').appendDtpicker({ 
		"closeOnSelected":true,
		"locale":"cn",
		"autodateOnStart":false,
		"futureOnly": true,
...
$(function(){
	$('input[id=date08]').appendDtpicker({ 
		"closeOnSelected":true,
		"locale":"cn",
		"autodateOnStart":false,
		"futureOnly": true,
