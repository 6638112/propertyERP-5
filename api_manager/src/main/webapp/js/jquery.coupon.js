    ...
    var itemScreen = {
		...
		changeUserRange: function(){
			...
			$(me.defaultOptions.selectChangeObj).change(function(){
				...
				if(curVal!=5){
					// 供应商
					$(".merchant-list-box").empty();
					$(".merchant-selected-box").empty().parent().find(".Validform_checktip").removeClass("Validform_right");
					// 小区信息
					$(".area-con .user-search-box:visible,.area-con .selected-box").empty();
					$(".area-con .Validform_checktip").removeClass("Validform_right");
					$(".addressAreaSelect").val("1").trigger('change').removeAttr("disabled");
				}
				//重置页面高度
		...
		itemSelectAndDelete: function(){
			...
			$(document).on("click", me.defaultOptions.selectObj, function(){ 
				...
				if(!$this.hasClass('on')){
					...
					var thisType = $this.find('.address-name').attr('data-type');
					//单选or多选
					...
					if(thisType==2){// 楼下店
						...
						$('#searchContent').val("");
						$(".user-area-search-btn").trigger('click');
						$(".area-con .selected-box").empty();
						$(".area-con .Validform_checktip").removeClass("Validform_right");
					}
				}else{//反选
						...
						case 0:
							...
							if(ssList.addressDesc){
								if(ssList.name.indexOf(currentVal) > -1 || ssList.addressDesc.indexOf(currentVal) > -1){
									//如果该店铺为体验店，设置标记
									if(ssList.isExperienceStore){
										searchList += '<li><span class="address-name is-the-store" data-itemid="' + ssList.id + '" data-type="'+ssList.type+'">' + ssList.name + '</span><br><span class="grey">地址：' + ssList.addressDesc + '</span></li>';
									}else{
										searchList += '<li><span class="address-name" data-itemid="' + ssList.id + '" data-type="'+ssList.type+'">' + ssList.name + '</span><br><span class="grey">地址：' + ssList.addressDesc + '</span></li>';
									}
								}								
							//城市搜索，不带详细地址
