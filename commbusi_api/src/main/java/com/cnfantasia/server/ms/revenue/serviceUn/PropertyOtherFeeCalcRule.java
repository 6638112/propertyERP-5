...
/**
 * 物业费实收金额收益计算
* Filename:    PropertyOtherFeeCalcRule.java
* @version:    1.0.0
 ...
 */
public class PropertyOtherFeeCalcRule extends AbstractRevenueCalcRuleByType<BigDecimal>{
	...
	public BigDecimal calcMoney(BigDecimal param, RevenueRole revenueRole, RevenueConfig revenueConfig) {
		...
		switch (role) {
			...
			/*case SysAdmin:
				...
				break;*/
			default:
				throw new BusinessRuntimeException("PropertyOtherFeeCalcRule.calcMoney.roleNotConfig",new Object[]{role.getCode()});
		}
		if(percentValue==null){
			throw new BusinessRuntimeException("PropertyOtherFeeCalcRule.calcMoney.percentValue.null");
		}
		//2.根据配置类型计算收益
		 ...
		 * 
			...
			throw new BusinessRuntimeException("PropertyOtherFeeCalcRule.calcMoney.ruleTypeNotSupport",new Object[]{ruleType});
		}
		return resultMoney;
	}
	...
	public BigDecimal getCalcInfo(RevenueRole revenueRole,String startTime, String endTime) {
		...
		switch (role) {
			case PropertyCompany://查询物业公司对应某时间段的用户实际缴费金额总额
				Double realPayMoneyTotal = revenueDao.selectPayBillOtherFeeTotal(revenueRole,startTime,endTime);
			...
			/*case PropertyAgent://查询代理商角色的验证门牌
				...
				break;*/
			default:
				throw new BusinessRuntimeException("PropertyOtherFeeCalcRule.getCalcInfo.roleNotSupport",new Object[]{role.getCode()});
		}
		return resCount;
	}
